...
Binary SFT 示例命令。服务器配置为 2 张 4090 48G、72 核 CPU、72G 内存时，使用 2 卡 DDP 训练：
...
这里的有效训练 batch size 为 `2 * 4 * 8 = 64`。如果训练时显存不足，优先把 `--per_device_train_batch_size` 和 `--per_device_eval_batch_size` 从 `4` 降到 `2`，其它参数保持不变。
