...
#
# 本脚本功能：
# - 评估 FakeAVCeleb MRDF subject-independent fold1 的 stage1 LoRA 模型；
...
OUTPUT_DIR="${OUTPUT_DIR:-/data/OneDay/OmniAV-Detect/outputs/batch_eval_binary/fakeavceleb_mrdf5fold_fold1_stage1_audio}"
GPUS="${GPUS:-0,1}"
NUM_WORKERS="${NUM_WORKERS:-2}"
BATCH_SIZE="${BATCH_SIZE:-1}"
FPS="${FPS:-2.0}"

LATEST_CKPT="$(find "${ADAPTER_ROOT}" -maxdepth 1 -type d -name 'checkpoint-*' | sort -V | tail -n 1)"
if [[ -z "${LATEST_CKPT}" ]]; then
  echo "No checkpoint found under: ${ADAPTER_ROOT}" >&2
  exit 1
fi
