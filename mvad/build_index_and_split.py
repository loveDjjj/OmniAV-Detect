...
def build_samples(unpack_root: Path) -> List[Dict[str, Any]]:
    """
    函数功能：
    - 扫描解压目录并构造 MVAD 样本列表。
    """
    samples = [parse_video_sample(path, unpack_root) for path in iter_video_files(unpack_root)]
    if not samples:
...
def write_split_outputs(samples: Sequence[Dict[str, Any]], train: Sequence[Dict[str, Any]], val: Sequence[Dict[str, Any]], work_root: Path) -> None:
    """写出索引、split 和统计文件。"""
    ...
    write_json(work_root / "split_stats.json", split_stats(train, val))
    write_json(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    samples = build_samples(Path(args.unpack_root))
    train, val = group_aware_split(samples, args.val_ratio, args.seed)
    write_split_outputs(samples, train, val, Path(args.work_root))
    logging.info("Built MVAD split: train=%d val=%d", len(train), len(val))
    return 0
